...
class Index_Builder:
    ...
    def encode_all(self):
        print('start loda encode data')
        encode_data = [item["contents"] for item in tqdm(self.corpus, desc="Extracting contents")]
        print('load encode data ends')
        if self.gpu_num > 1:
            print("Use multi gpu!")
    ...
    def build_dense_index(self):
        ...
        if self.is_clip:
            ...
            self.save_faiss_index(all_embeddings, self.faiss_type, self.index_save_path)
        print("Finish!")
    def select_train_vectors(self, embeddings, train_size):
            """选择训练向量，确保数据具有代表性"""
            ntotal = len(embeddings)
            if ntotal <= train_size:
                return embeddings
            
            # 随机选择，但确保均匀分布
            indices = np.random.choice(ntotal, train_size, replace=False)
            indices.sort()  # 排序以提高内存访问效率
    ...
    def save_faiss_index(
        ...
        if('ivf' in faiss_type):
            nlist = int(np.sqrt(all_embeddings.shape[0]))
        ...
        if self.faiss_gpu:
            ...
            if not faiss_index.is_trained:
                ntotal = len(all_embeddings)
                train_size = min(1000000, ntotal)
                print(f"\n选择训练数据:")
                print(f"- 总向量数: {ntotal}")
                print(f"- 训练向量数: {train_size}")
                print(f"- 采样比例: {train_size/ntotal*100:.1f}%")
                train_vectors = self.select_train_vectors(all_embeddings, train_size)
