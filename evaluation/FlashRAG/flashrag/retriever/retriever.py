...
class RemoteRetriever(BaseTextRetriever):
    def __init__(self, config: dict):
        super().__init__(config)
        self.remote_url = config["remote_retriever_url"] # "http://0.0.0.0:"+port
    
    @retry(max=10, sleep=1) # 重试机制，如果请求失败，则重试10次，每次等待1秒
    def _batch_search(self, query: list, num: int = None,  return_score: bool = False):
        url = f"{self.remote_url}/queries"
        try:
            print('打印请求的', query[0])
            response = requests.post(url, json={"queries": query, "k": num})
            print('请求结束')
            print('打印服务器返回的', response.json()["queries"][0], response.json()["answers"][0])
            all_answers = response.json()["answers"]
            all_scores = []
            for answer in all_answers:
                temp_scores = []
                for j in range(len(answer)):
                    temp_scores.append(1.0 - (j * 0.1))
                all_scores.append(temp_scores)
        except Exception as e:
            print(f"Error in _batch_search: {e}")
            return [], []
        if return_score:
        ...
        # for q in query:
        #     temp_answer = []
        #     temp_scores = []  # 每个答案对应的分数
        #     for j in range(num or 3):
        #         temp_answer.append("false answer")
        #         temp_scores.append(1.0 - (j * 0.1))  # 模拟分数，越靠前分数越高
        #     all_answers.append(temp_answer)
        #     all_scores.append(temp_scores)
    ...
    # def _batch_search(self, query: List[str], num: int = None):
    #     url = f"{self.remote_url}/batch_search"
    #     response = requests.post(url, json={"query": query, "k": num})
    #     return response.json()
