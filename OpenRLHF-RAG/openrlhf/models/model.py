    ...
    **kwargs,
    ...
    if init_value_head:
        ...
        if dschf is not None:
            logger.info("initialize value_head for ZeRO-3 reward model training.") #ZeRO-3 会将 value_head.weight 的参数存储在 CPU，而不是 GPU，所以需要特殊方式初始化它。
            with deepspeed.zero.GatheredParameters([value_head.weight], modifier_rank=0): #临时聚合参数（gather）到 rank=0 进程上。确保 rank=0 的 GPU 进程能够访问完整的参数，以便正确初始化 value_head.weight。
                if torch.distributed.get_rank() == 0: #确保只有 rank=0 的 GPU 进程执行初始化。
...
def _get_reward_model(base_pretrained_model, base_llm_model, value_head_prefix="score", packing_samples=False):
    #为模型添加一个 value_head（线性层），用于回归奖励值。支持 Packing 训练，提高训练效率。
    class RewardModel(base_pretrained_model):
        ...
        def __init__(self, config: AutoConfig):
            ...
            self.register_buffer("mean", torch.zeros(1), persistent=False) #register_buffer: mean 和 std 不属于 model.parameters()，不会被优化器更新。持久性设置 persistent=False，避免 state_dict 存储。
            self.register_buffer("std", torch.ones(1), persistent=False)

            # load mean/std from config.json
            if hasattr(config, "mean"): #如果 config.json 里提供了 mean/std，则使用 预设的均值和标准差。
        ...
        def forward(
            ...
            # 对使用packing和不适用packing 分别计算position 和attention_mask
            ...
            values = getattr(self, self.value_head_prefix)(last_hidden_states).squeeze(-1) #(batch_size, sequence_length)

            if self.packing_samples: #如果是packing，只取用eos token上的reward
                if ring_attn_group is not None: #在 分布式训练 时，每个 GPU 只持有一部分 input_ids，如果不进行 跨 GPU 同步，会导致 attention mask 计算错误。ring_attn_group 允许不同 GPU 交换 position_ids 和 attention_mask，确保计算一致。
