...
class PolicyLoss(nn.Module):
    ...
    def forward(
        self,
        log_probs: torch.Tensor,         # shape: [batch_size, seq_len]，当前策略的 log prob（对每个 action）
        old_log_probs: torch.Tensor,     # shape: [batch_size, seq_len]，旧策略的 log prob（行为来自旧策略）
        advantages: torch.Tensor,        # shape: [batch_size, seq_len]，advantage 值
        action_mask: Optional[torch.Tensor] = None,  # shape: [batch_size, seq_len]，标记哪些位置是有效 action（通常是 padding mask）
        retrieve_mask: Optional[torch.Tensor] = None # shape: [batch_size, seq_len]，可能用于额外屏蔽掉非检索生成的 token
    ) -> torch.Tensor:
        assert retrieve_mask is not None, "retrieve_mask should not be None"


        ratio = (log_probs - old_log_probs).exp() #恒大于0
        print('打印ratio', ratio)
        surr1 = ratio * advantages
        surr2 = ratio.clamp(1 - self.clip_eps, 1 + self.clip_eps) * advantages #    shape: [batch_size, seq_len]   # 第二项：clip ratio，限制策略变化不超过 ±20%，防止过度优化

        loss = -torch.min(surr1, surr2) #        # PPO 的核心目标：对每个 token 取两个 surrogate 目标的最小值（更保守），再乘上负号变成 loss
        print('打印loss', loss)
        loss = masked_mean(loss, action_mask, retrieve_mask, dim=-1).mean()
