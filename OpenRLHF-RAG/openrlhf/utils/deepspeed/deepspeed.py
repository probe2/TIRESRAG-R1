...
class DeepspeedStrategy(ABC):
    ...
    #     )
    def setup_distributed(self, timeout=timedelta(minutes=5)) -> None:
        self.set_seed(self.seed)
        ...
        # Initializes the distributed backend which will take care of sychronizing nodes/GPUs
        print(f"Before init_distributed: rank={local_rank}, world_size={os.environ.get('WORLD_SIZE')}")
        deepspeed.init_distributed(timeout=timeout)
        print("After init_distributed")
        self.setup_ring_attn()
        self.world_size = dist.get_world_size()
        self.accumulated_gradient = (
            self.train_batch_size * self.ring_attn_size // self.micro_train_batch_size // self.world_size
    ...
    def _ds_init_train_model(self, model, optim, scheduler):
        ...
        engine, optim, _, scheduler = deepspeed.initialize(
            ...
            config=ds_config,
            args={"local_rank": int(os.environ.get("LOCAL_RANK", "-1"))}, #int(os.environ.get("LOCAL_RANK", "-1"))}, #self.args.local_rank},
