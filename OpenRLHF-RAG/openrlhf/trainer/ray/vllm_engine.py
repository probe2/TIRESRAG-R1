...
class LLMRayActor:
    ...
    def __init__(self, *args, bundle_indices: list = None, **kwargs):
        ...
        if kwargs.get("distributed_executor_backend") == "ray":
            ...
            print('打印os.environ[CUDA_VISIBLE_DEVICES]',os.environ['CUDA_VISIBLE_DEVICES'])
        ...
        if bundle_indices is not None:
            ...
            print('打印bundle_indices',bundle_indices, num_gpus)
    ...
    def update_weight_directly(self, state_dict):
        return self.llm.collective_rpc("update_weight_directly", args=(state_dict))
    ...
    def sleep(self, level=1):
        ...
        print('打印sleep level',level)
    def wake_up(self):
        self.llm.wake_up()
    def get_param_stats(self):
        # 适配不同的 model_executor 类型
        model_executor = self.llm.llm_engine.model_executor
        model = None
        if hasattr(model_executor, "model"):
            model = model_executor.model
        elif hasattr(model_executor, "driver_worker"):
            driver_worker = model_executor.driver_worker
            # 优先查找 driver_worker.model
            if hasattr(driver_worker, "model"):
                model = driver_worker.model
            # 再查找 driver_worker.worker.model_runner.model
            elif hasattr(driver_worker, "worker") and hasattr(driver_worker.worker, "model_runner") and hasattr(driver_worker.worker.model_runner, "model"):
                model = driver_worker.worker.model_runner.model
        if model is None:
            raise AttributeError("Cannot find model in model_executor!")
        stats = []
...
def create_vllm_engines(
    ...
    for i in range(num_engines):
        ...
        print('打印vllm ray options num_gpus',num_gpus)
        vllm_engines.append(
            LLMRayActor.options(
                num_cpus=num_gpus,
                num_gpus=num_gpus,
